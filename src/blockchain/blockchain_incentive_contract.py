...
class BlockchainIncentiveContract:
    ...
    def submit_model_contribution(self, participant_address: str, round_number: int,
        ...
        try:
            ...
            # Use the funded account for transactions
            from_address = self.aggregator_address if hasattr(self, 'use_funded_account_mode') and self.use_funded_account_mode else self.account.address
    ...
    def distribute_token_rewards(self, round_number: int, 
        ...
        try:
            ...
            amounts = [dist.token_amount for dist in reward_distributions]
            
            # Use the funded account for transactions
            from_address = self.aggregator_address if hasattr(self, 'use_funded_account_mode') and self.use_funded_account_mode else self.account.address
            
            # Build transaction - distribute rewards individually since distributeTokenRewards doesn't exist
            # For now, we'll distribute to the first recipient as a simplified approach
...
class BlockchainIncentiveManager:
    ...
    def process_round_contributions(self, round_number: int, 
        ...
        for contribution in client_contributions:
            try:
                ...
                success = self.contract.submit_model_contribution(
                    client_address, round_number, metrics
                )
                ...
                if success:
                    ...
                    contribution_score = int(metrics.accuracy_improvement * 100)  # Convert to percentage
                    token_reward = int(contribution_score * 10)  # 10 tokens per percentage point
                    
                    eval_success = self.contract.evaluate_contribution(
                        client_address, round_number, contribution_score, token_reward, verification_result
                    )
                    
                    if eval_success:
                        # Get contribution info to calculate reward
                        contrib_info = self.contract.get_contribution_info(round_number, client_address)
    ...
    def distribute_rewards(self, round_number: int, reward_distributions: List[RewardDistribution]) -> bool:
        ...
        try:
            ...
            success = self.contract.distribute_token_rewards(round_number, reward_distributions)
