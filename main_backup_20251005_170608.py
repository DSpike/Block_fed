...
@dataclass
    ...
    def run_federated_training_with_incentives(self) -> bool
        ...
        try
            ...
            for round_num in range(1, self.config.num_rounds + 1)
                logger.info(f"n🔄 ROUND {round_num}/{self.config.num_rounds}")
                logger.info("-" * 50)
    ...
    def _get_client_training_accuracy(self, client_id: str) -> float
        """
    ...
    def _calculate_shapley_values(self, client_updates: List, current_accuracy: float) -> Dict[str, float]
        """
        Calculate Shapley values for fair contribution evaluation.

        Args
            client_updates: List of client updates from the round
            current_accuracy: Current round accuracy

        Returns
            Dictionary mapping client_id to Shapley value
        """
        try
        ...
        for client_update in client_updates
                ...
                # Get real performance data from training history
                # Extract the actual training accuracy from the latest round's training logs
                individual_performances[client_id] = self._get_client_training_accuracy(client_id)

                # Calculate data quality based on sample count and loss
                sample_count = getattr(client_update, 'sample_count', 1000)
                training_loss = getattr(client_update, 'training_loss', 0.5)

                # Data quality: higher sample count and lower loss = better quality
                data_quality_score = min(0.95, 0.7 + (sample_count / 2000) * 0.2 + (0.5 - training_loss) * 0.1)
                client_data_quality[client_id] = data_quality_score

                # Participation rate (assume full participation for now)
                client_participation[client_id] = 1.0

            # Debug: Log the values being passed to Shapley calculator
            logger.info(f"Shapley calculation inputs:")
            logger.info(f"  Global performance: {current_accuracy}")
            logger.info(f"  Individual performances: {individual_performances}")
            logger.info(f"  Data quality: {client_data_quality}")

            # Calculate Shapley values
            contributions = shapley_calculator.calculate_shapley_values(
                global_performance=current_accuracy,
                individual_performances=individual_performances,
                client_data_quality=client_data_quality,
                client_participation=client_participation
            )

            # Debug: Log the calculated Shapley values
            logger.info(f"Calculated Shapley values:")
        for contrib in contributions
                logger.info(f"  {contrib.client_id}: {contrib.shapley_value:.6f}")

            # Return Shapley values as dictionary
            shapley_values = {contrib.client_id: contrib.shapley_value for contrib in contributions}

            logger.info("Shapley Value Results:")
        for client_id, shapley_value in shapley_values.items()
                logger.info(f"  {client_id}: Shapley={shapley_value:.4f}")
            ...
            return {client_update.client_id: 1.0/len(client_updates) for client_update in client_updates}
    ...
    def _process_round_incentives(self, round_num: int, round_results: Dict, 
        ...
        try
            ...
            logger.info(f"Processing Shapley value-based incentives for round {round_num}")

            # Get client updates from round results
            client_updates = round_results.get('client_updates', [])
        if not client_updates
                logger.warning(f"No client updates found for round {round_num}")
                return

            # Calculate Shapley values for fair contribution evaluation
            shapley_values = self._calculate_shapley_values(client_updates, current_accuracy)
        ...
        if hasattr(client_update.model_parameters, 'state_dict')
                    ...
                    # Use Shapley value for contribution score
                    shapley_value = shapley_values.get(client_update.client_id, 0.0)
                    contribution_score = int(shapley_value * 100)  # Scale Shapley value for logging
            ...
            if client_contributions
                ...
                if reward_distributions
                    ...
                    if success
                        ...
                        logger.info(f"Shapley-based incentives processed for round {round_num}: {len(reward_distributions)} rewards, Total: {total_tokens} tokens")

                        # Store incentive data with Shapley values
                        incentive_record = {
    ...
    def get_incentive_summary(self) -> Dict[str, Any]
        ...
        try
            ...
            # Calculate participant rewards based on actual Shapley-based distributions
        if self.incentive_history
        ...
        for address in client_addresses[:self.config.num_clients]
        ...
        if client_num < len(client_addresses)
                                    ...
                                    summary['participant_rewards'][address] += reward_share
        else
                        # Fallback to equal distribution if Shapley values not available
                        total_round_rewards = record['total_rewards']
                        reward_per_client = total_round_rewards / self.config.num_clients
        for address in client_addresses[:self.config.num_clients]
                            summary['participant_rewards'][address] += reward_per_client
            
            # Get round summaries
            for record in self.incentive_history
        if self.incentive_manager is not None

                round_summary = self.incentive_manager.get_round_summary(record['round_number'])
                summary['round_summaries'].append(round_summary)
        else
                    # Fallback when incentive_manager is not available
                    summary['round_summaries'].append({
                        ...
                        'total_tokens_distributed': record['total_rewards']
                    })
    ...
    # Create enhanced system configuration (using
    
    class defaults with increased training)
